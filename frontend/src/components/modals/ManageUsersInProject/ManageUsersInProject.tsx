...
const OneRow = memo(function ({
    user,
    handleDelete,
    handleChange,
}: {
    user: UserDataRolesDTO;
    handleDelete: (id: number) => void;
    handleChange: (id: number, role: Role) => void;
}) {
    return (
        <div className={css.onerow}>
            ...
            <div className={css.tools}>
                <div className={css.select}>
                    <BlueSelect
                        onChange={e => handleChange(user.id, e.target.value as unknown as Role)}
                    >
                        <option selected={user.role == Role.user} value={Role.user}>
                            {'Участник'}
                        </option>
...
export default function ManageUsersInProject({ projectId, isOpen, handleClose }: Props) {
    ...
    return (
        <ModalBase isOpen={isOpen} onRequestClose={() => handleClose()}>
                ...
                {localUsers.map(us => (
                    <OneRow
                        ...
                        handleDelete={(id: number) =>
                            setLocalUsers(state => state.filter(us => us.id !== id))
                        }
                        handleChange={(id: number, role: Role) =>
                            setLocalUsers(state =>
                                state.map(us => (us.id === id ? { ...us, role: role } : us)),
                            )
                        }
                    />
                ))}
