...
import ApiQuery from './QueryController';
...
authInstance.interceptors.response.use(
    ...
    async error => {
        ...
        if (
            localStorage.getItem(core.localStorageKeys.access_token) &&
            error.response &&
            ...
            try {
                ...
                if (
                    innerError &&
                    (innerError as any).response &&
                    (innerError as any).response.status === 401
                ) {
