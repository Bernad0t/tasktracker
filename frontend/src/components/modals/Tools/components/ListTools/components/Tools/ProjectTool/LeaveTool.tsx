...
export default function LeaveTool({projectId, ...props}: IToolProps){
    ...
    const handleClick = () => {
        dispatch(ProjectSliceManager.fetching.leaveProject(projectId))
        .then(() => toolManager?.handleClose())
    }
    return(
        <ListToolBase img={imageLeave} label="Покинуть" {...props} onClick={handleClick}/>
    )
}