...
export default function InfoTaskModal({task, isOpen, handleClose}: {task: TaskDTORelation, isOpen: boolean, handleClose: () => void}){
    ...
    const handleSend = (newcomm: CommentCreateDTO) => {
        if (inputRef.current && project){
            ApiQuery.task.sendComm(newcomm)
            .then((id) => dispatch(ProjectSliceManager.redusers.updateSelect({
                ...project, 
                tasks: project.tasks?.map(
                    projtask => projtask.id === task.id ? 
                    {...task, comments: [...(task.comments??[]), {...newcomm, id: id}]}:
                    projtask
                )
            })))
            ...
            <div className={css.wrapper}>
                ...
                <div className={css.panel}>
                    <div className={css.block}>
                        ...
                        <div className={css.description}>
                            <div>
                                <b>Назначил:</b> {project?.users?.find(us => us.id === task.reviewer)?.username}
                            </div>
                            <div>
                                <b>Исполнитель:</b> {project?.users?.find(us => us.id === task.assigned)?.username} 
                            </div>
                        </div>
                    ...
                    <div className={css.chatBlock}>
                        <div className={css.chatWrap}>
                            {project && task.comments?.map((comm, index) => 
                                <ContextDayMessage 
                                    key={comm.id}
                                    curMes={comm}
