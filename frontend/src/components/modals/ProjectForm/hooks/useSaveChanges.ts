...
export default function useSaveChanges(typeManipulate: TypeManipulateWihProjectForm) {
    ...
    const handleSubmit = useCallback(
        (data: ProjectListAdapted | ProjectBaseDTO) => {
            if (data.name.length !== 0) {
                const fetch = async () => {
                    if (typeManipulate === TypeManipulateWihProjectForm.add) {
                        await dispatch(ProjectSliceManager.fetching.addProject(data));
                    } else {
                        // это вызывается только при обновлении проекта, то есть от ProjectListAdapted
                        await dispatch(
                            ProjectSliceManager.fetching.updateProject(data as ProjectListAdapted),
                        );
                    }
                };
                setIsLoading(true);
