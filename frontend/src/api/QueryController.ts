...
class ApiQueryClass {
    authorization = {
        async enter(data: AuthorizationProp) {
            await axios
                .post(core.serverEndnpoints.auth.enterAuth, data, { withCredentials: true })
                .then(({ data }) =>
                    localStorage.setItem(core.localStorageKeys.access_token, data.accessToken),
                );
        },

        async register(data: RegisrationProp) {
            await axios
                .post(core.serverEndnpoints.auth.regAuth, data, { withCredentials: true })
                .then(({ data }) =>
                    localStorage.setItem(core.localStorageKeys.access_token, data.accessToken),
                );
        },
    ...
    user = {
        ...
        async findUsers(login: string, signal?: GenericAbortSignal | undefined) {
            const data =
                (
                    await authInstance.get<UserDataDTO[] | undefined>(
                        core.serverEndnpoints.user.find,
    ...
    project = {
        ...
        async deleteProject(id: number) {
            await authInstance.delete(core.serverEndnpoints.project.delete, {
                params: { idProject: id },
            });
        },
        async addProject(project: ProjectBaseDTO, ownerId: number) {
            const idNewProject = await authInstance
                .post(core.serverEndnpoints.project.add, {
                    ...project,
                    users: [{ id: ownerId, role: Role.admin }],
                })
        ...
        async leave(projectId: number) {
            await authInstance.delete(core.serverEndnpoints.project.leave, {
                params: { projectId },
            });
        },
        async getSelectedProject(projectId: number): Promise<ProjectDTORelation> {
            const project = (
                await authInstance.get(core.serverEndnpoints.project.getSelectedProject, {
                    params: { projectId },
                })
            ).data;
            return project;
    ...
    task = {
        async addTask(task: TaskDTO): Promise<number> {
            const taskId = (await authInstance.post(core.serverEndnpoints.task.add, task)).data
                .taskId;
            return taskId;
        },
        async sendComm(comm: CommentCreateDTO) {
            const sendedCommId = (await authInstance.post(core.serverEndnpoints.comm.send, comm))
                .data.commId;
            return sendedCommId;
        },
